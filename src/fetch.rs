...
pub struct Problems {
    ...
    ac_medium:             u32,
    ac_hard:               u32,
    pub stat_status_pairs: Vec<Status>,
}
...
impl Problems {
    pub async fn new() -> Option<Problems> {
    ...
    pub async fn get_problem_by_id(&self, fronted_question_id: u32) -> Option<Problem> {
        for status in self.stat_status_pairs.iter() {
            if status.stat.frontend_question_id == fronted_question_id {
                if status.paid_only {
                    return None;
                }
                ...
                let resp: RawProblem = client
                    ...
                    .json(&Query::question_query(
                        status.stat.question_title_slug.as_ref().unwrap(),
                    ))
                    .send()
                    .await
                    .unwrap()
                ...
                return Some(Problem {
                    title:            status.stat.question_title.clone().unwrap(),
                    title_slug:       status.stat.question_title_slug.clone().unwrap(),
                    code_definition:  serde_json::from_str(&resp.data.question.code_definition)
                        .unwrap(),
                    content:          resp.data.question.content,
                    sample_test_case: resp.data.question.sample_test_case,
                    difficulty:       status.difficulty.to_string(),
                    question_id:      status.stat.frontend_question_id,
                    return_type:      {
